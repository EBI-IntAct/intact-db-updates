...
public class UniprotProteinRetrieverListener extends AbstractProteinUpdateProcessorListener {
    /**
     * UniProt Data Source.
     */
    ...
    public UniprotProteinRetrieverListener(UniprotService uniprotService) {
            ...
            if(uniprotProteins.size() == 0){
                ...
                if ( 1 == getSpeciesCount( uniprotProteins ) ) {
                    ...
                    if (IdentifierChecker.isSpliceVariantId(uniprotAc)){
                        UniprotProtein originalParent = null;
                        ...
                        if (proteinsWithSameBaseUniprotAc.size() == 1){
                            evt.setUniprotProtein(proteinsWithSameBaseUniprotAc.iterator().next());
                        }
                    }
                    if (evt.getUniprotProtein() == null){
                        ...
                        if (evt.getSource() instanceof ProteinUpdateProcessor) {
                            final ProteinUpdateProcessor updateProcessor = (ProteinUpdateProcessor) evt.getSource();
                            updateProcessor.fireonProcessErrorFound(new UpdateErrorEvent(updateProcessor, evt.getDataContext(), "No uniprot entry is matching the ac " + uniprotAc, UpdateError.several_uniprot_entries_same_organim));
                        }
                    ...
                    if (proteinsWithSameTaxId.size() == 1){
                        evt.setUniprotProtein(proteinsWithSameTaxId.iterator().next());
                    }
                    else {
                        // several splice variants can be attached to several master proteins and it is not an error. If we are working with such protein transcripts, we need to update them
                        if (IdentifierChecker.isSpliceVariantId(uniprotAc)){
                        ...
                        if (evt.getUniprotProtein() == null){
                            ...
                            if (evt.getSource() instanceof ProteinUpdateProcessor) {
                                final ProteinUpdateProcessor updateProcessor = (ProteinUpdateProcessor) evt.getSource();
                                updateProcessor.fireonProcessErrorFound(new UpdateErrorEvent(updateProcessor, evt.getDataContext(), "No uniprot entry is matching the ac " + uniprotAc, UpdateError.several_uniprot_entries_different_organisms));
                            }
                ...
                evt.setUniprotProtein(uniprotProteins.iterator().next());
            }
        }
    }
    ...
    public void onSecondaryAcsFound(UpdateCaseEvent evt)  throws ProcessorException {
        ...
        // the protein has been fetched earlier by the UniprotPrimaryAcUpdater
        if (serviceResult != null){
            if (secondaryProteins != null){
                ...
                throw new ProcessorException("the list of secondary proteins should not be null.");
            }
        }
        else {
            throw new ProcessorException("the UniprotServiceResult should not be null to be able to retrieve the uniprot entries of secondary proteins. It contains the uniprot primary ac of the protein which is updated at the moment.");
        }
    }
